  ...
  return (
    <>
      ...
      <aside
        ...
        className="fixed left-0 top-0 h-screen z-50 flex flex-col transition-all duration-300 ease-[cubic-bezier(0.4,0,0.2,1)] shadow-2xl"
        style={{
      ...
      >
        {/* Brand Header */}
        <div className="px-5 py-6 flex items-center justify-between min-h-[80px]">
          <div className={`flex items-center gap-3 overflow-hidden transition-all duration-300 ${!isExpanded ? "w-0 opacity-0" : "w-auto opacity-100"}`}>
          ...
          {/* Collapse toggle - only visible when NOT hovered (to avoid flickering) OR if fully expanded */}
          <button
            onClick={(e) => {
              e.stopPropagation();
            ...
            className={`w-8 h-8 rounded-lg flex items-center justify-center text-indigo-400 hover:text-white hover:bg-white/10 transition-all flex-shrink-0 ${!isExpanded ? "mx-auto" : ""}`}
          >
        ...
        <nav className="flex-1 px-3 py-4 space-y-1 overflow-y-auto">
          {/* Section label */}
          <p className={`text-indigo-500 text-[10px] font-semibold uppercase tracking-widest px-3 mb-2 transition-opacity duration-200 ${!isExpanded ? "opacity-0" : "opacity-100"}`}>
            Menu
          </p>
          {navItems.map((item, index) => {
            ...
            return (
              ...
              >
                ...
                <span className={`transition-all duration-300 whitespace-nowrap ${!isExpanded ? "w-0 opacity-0 overflow-hidden" : "w-auto opacity-100"}`}>
                  {item.label}
                </span>
                {isActive && isExpanded && (
        ...
        <div className="px-3 pb-2">
          ...
          <Link
            to="/settings"
            className={`group flex items-center gap-3 px-3 py-2.5 rounded-xl text-[13px] font-medium text-slate-400 hover:text-white hover:bg-white/[0.06] transition-all ${!isExpanded ? "justify-center" : ""}`}
        ...
        {/* User card */}
        <div className={`px-3 pb-5 ${!isExpanded ? "px-2" : ""}`}>
          <div
            className={`flex items-center gap-3 px-3 py-3 rounded-xl transition-all ${!isExpanded ? "justify-center px-0" : ""}`}
